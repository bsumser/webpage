    ...
    <div className="relative group p-2 rounded-md shadow-sm bg-gray-100 border border-gray-200">
      ...
      {imageUrl && (
        <img
          src={imageUrl}
          alt={card.name}
          className="absolute z-50 top-0 left-full ml-4 w-52 md:w-60 hidden group-hover:block rounded-xl shadow-2xl border-2 border-gray-400 pointer-events-none transition-all duration-200"
        />
      )}
    </div>
  );
